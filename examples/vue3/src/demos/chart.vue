...
const pieChartData = {
	...
	datasets: [{
		data: [55, 35, 10],
		backgroundColor: ['#667eea', '#42b883', '#f59e0b']
	}]
}

const chartType = ref<'bar' | 'line' | 'pie'>('bar')

const basicCode = `<canvas v-chart="{
  type: 'bar',
  labels: ['Jan', 'Feb', 'Mar'],
  datasets: [{
    label: 'Sales',
...
const optionsCode = `<canvas v-chart="{
  type: 'line',
  labels: ['Mon', 'Tue', 'Wed'],
  datasets: [{
    label: 'Visitors',
...
const composableCode = `import { useChart } from 'directix'
