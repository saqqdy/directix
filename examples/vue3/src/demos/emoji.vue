...
const basicCode = `<!-- Strip all emojis -->
<input v-emoji type="text" v-model="text" />`

const replacementCode = `<input v-emoji="{ strip: true, replacement: '*' }" type="text" />`

const allowCode = `<!-- Allow specific emojis -->
<input v-emoji="{ allowList: ['😊', '👍'] }" type="text" />

<!-- Only 😊 and 👍 will be allowed, others stripped -->`

const blockCode = `<!-- Block specific emojis -->
<input v-emoji="{ blockList: ['🚫', '❌'] }" type="text" />

<!-- 🚫 and ❌ will be stripped, others allowed -->`

const composableCode = `import { useEmoji } from 'directix'
		...
		<DemoSection title="Composable API - useEmoji" description="Using useEmoji composable">
			<div class="demo-box">
				...
				<div class="button-group">
					...
					<button @click="alert(containsEmoji(composableText) ? 'Contains emoji!' : 'No emoji')" class="btn btn-secondary">
						Check for Emoji
					</button>
				</div>
