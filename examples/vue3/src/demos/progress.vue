...
// Composable API demo
const composableProgress = ref(0)
const { setProgress, complete } = useProgress({
	value: 0,
	color: '#667eea',
	onChange: (val) => composableProgress.value = val
})

const basicCode = `<div v-progress="50">
  Progress at 50%
</div>
...
const optionsCode = `<div v-progress="{
  value: progressValue,
  color: '#42b883',
  height: 8,
  showText: true,
		...
		<DemoSection title="Composable API - useProgress" description="Using useProgress composable">
			<div class="demo-box">
				<div class="button-group">
					<button @click="setProgress(25)" class="btn">25%</button>
					<button @click="setProgress(50)" class="btn">50%</button>
					<button @click="setProgress(75)" class="btn">75%</button>
					<button @click="complete()" class="btn btn-secondary">Complete</button>
				</div>
				<p class="hint">Current: {{ composableProgress }}%</p>
			</div>
