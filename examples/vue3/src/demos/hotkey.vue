...
const handleUndo = () => {
	hotkeyLog.value.unshift('Undo (Ctrl+Z)')
	if (hotkeyLog.value.length > 5) hotkeyLog.value.pop()
}
const handleRedo = () => {
	hotkeyLog.value.unshift('Redo (Ctrl+Y)')
	if (hotkeyLog.value.length > 5) hotkeyLog.value.pop()
}
const handleBold = () => {
	hotkeyLog.value.unshift('Bold (Ctrl+B)')
	if (hotkeyLog.value.length > 5) hotkeyLog.value.pop()
}
const handleItalic = () => {
	hotkeyLog.value.unshift('Italic (Ctrl+I)')
	if (hotkeyLog.value.length > 5) hotkeyLog.value.pop()
}

// Scenario 4: Multiple hotkeys in object syntax
...
const handleSearch = () => {
	if (searchText.value) {
		searchResults.value = [`Result for: ${searchText.value}`, `Another result for: ${searchText.value}`]
	}
}
const handleClear = () => {
	searchText.value = ''
...
const hotkeysDisabled = ref(false)
const disabledCount = ref(0)
const handleDisabledHotkey = () => {
	disabledCount.value++
}

const basicCode = `<div v-hotkey:escape="handleEscape">
  Press Escape to trigger
</div>`

const modifierCode = `<!-- Ctrl + S -->
...
const multipleCode = `<!-- Using array syntax -->
<div v-hotkey="[
...
const objectCode = `<!-- Using object syntax -->
<div v-hotkey="{
  'ctrl+z': handleUndo,
  'ctrl+y': handleRedo,
  'ctrl+b': handleBold
