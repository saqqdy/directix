...
export default defineComponent({
	...
	setup() {
		...
		// Vue 2 对 Set 响应式支持不完善，使用数组代替
		const visibleItems = ref<boolean[]>([false, false, false, false, false, false])
		...
		const onDrop = (e: DragEvent, targetIndex: number) => {
			...
			const draggedVisible = newVisible[draggedIndex.value]
			newVisible.splice(draggedIndex.value, 1)
			newVisible.splice(targetIndex, 0, draggedVisible)
		...
		const handleIntersect = (index: number) => (entry: IntersectEntry) => {
			// 强制触发响应式更新
			const newArr = [...visibleItems.value]
			newArr[index] = entry.isIntersecting
			visibleItems.value = newArr
		}
		...
		// 自由拖拽卡片位置 - 使用 reactive 对象
		const cardPositions = reactive<Record<number, CardPosition>>({
			1: { x: 20, y: 20 },
			2: { x: 180, y: 60 },
			3: { x: 100, y: 120 },
		})

		const updateCardPosition = (id: number, pos: CardPosition) => {
			cardPositions[id] = { x: pos.x, y: pos.y }
		}
	...
	<div class="scenario-container">
		...
		<div class="demo-section">
			...
			<div class="visibility-status">
				<span
					v-for="(visible, i) in visibleItems"
					:key="i"
					class="status-dot"
			...
			<div class="card-grid">
				<div
					...
					:style="{
						...
						transform: 'translate(' + cardPositions[item.id].x + 'px, ' + cardPositions[item.id].y + 'px)',
					}"
				>
